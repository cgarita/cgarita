...
#INICIO
filelist = os.listdir('/autoprueba/')
filelist = filter(lambda x: not os.path.isdir(x), filelist)
newest = max(filelist)
newest = '/autoprueba/' + newest
#f = file(newest, 'r')
#####
f = open(newest, 'r+')
li = f.readline()
...
try:
		...
		sys.exit()
 	 	...
 	 	if restmpnow < 10 or mg != ML:
			#try:
				...
				sismoep = 'ACTUALIZADO:%20MAG:%20'+ str(ML) + '%20FECHA:' + fechalocal[0] + '%20' + horalocal[0]
				...
				#Guarda nuevo archivo
					...
					db.close()
					#os.system('/usr/bin/python /opt/ProgramaEQ/leereqmapa.py')
					#subprocess.call('/opt/ProgramaEQ/leereqmapa.py')
					#subprocess.call('/opt/ProgramaEQ/test.py')
					sismo = fechalocal[0] + '/' + horalocal[0] + '/'+ str(ML) + '/'+ str(depth) + '/'+ str(lati) + '/'+ str(longi) + '/'+ localizacion 	
					# Creando archivo feeder de los sismos para APP
					...
					#feed.write(sismo +"/U\n")
					#feed.close
					#Para el APP Epicentro
					########urllib2.urlopen("http://www.intraovsi.una.ac.cr/movilnotification/notification_sender.php?pushMessage="+ sismoep)
					#subprocess.call('php /var/www/movilnotification/notification_sender.php?pushMessage='+ sismo)
					#sys.exit()-----------31/01/2013
					upd = 2
				except:
	...
	print 'Creando archivo /tmp/tmpeqmw'
	tmp = open('/tmp/tmpeqmw','wb')
	tmp.write(fecha +','+ hora + ',' + str (ML))
	tmp.close()	
...
# Commit your changes in the database
		...
		urllib2.urlopen("http://www.intraovsi.una.ac.cr/movilnotification/notification_sender.php?pushMessage="+ sismoep)
		#os.system('/usr/bin/python /opt/ProgramaEQ/leereqmapa.py')
		#subprocess.call('/usr/bin/python2.6 /opt/ProgramaEQ/leereqmapa.py')
		#subprocess.call('/opt/ProgramaEQ/test.py')
	except:
# Rollback in case there is any error
		db.rollback()
