...
def notificar(action,ev,localizacion):
    ...
    with open("/opt/feed/feed.txt", "r+") as f:
        ...
        f.seek(0);
        #f.write(sismo +"/A\n" + s)
    # Enviar notificaciones a celulares (no esta sirviendo)	
    url = "http://www.intraovsi.una.ac.cr/movilnotification/notification_sender.php?pushMessage="
    url += sismo
    urllib2.urlopen(url)

    # twitter y facebook
    urlsurvey = "http://www.ovsicori.una.ac.cr/index.php?option=com_wrapper&view=wrapper&Itemid=121&fechalocal=" + dia +"&horalocal=" + hora
    twitter = "%s: %s/%s MAG: %s %s PROF:%skm / %s /Lo sentiste? %s" % (action, dia, hora,
            mag,ev['magtype'], depth, localizacion, urlsurvey)
...
for ev in j.keys():
    ...
    if total == 1:
    	actualiza = 1
	#Variables para consultar por tiempo, mysql entiende 9:20 no 09:20
    	mysqlhrar = str(resultados[0]['horalocal']) 
    	jsonhrar = str(event['horaLocal'])
    	mysqlhra= mysqlhrar.split(':')
    	jsonhra= jsonhrar.split(':')
    	if int(mysqlhra[0]) < 10:
		mysqlhrastr = '0'+mysqlhra[0]
		timesql= mysqlhrastr + ':'+ mysqlhra[1] + ':' + mysqlhra[2]
    	else:
		mysqlhrastr = str(mysqlhra[0])
		timesql = str(resultados[0]['horalocal']) 
	...
	if str(resultados[0]['fechalocal']) == str(event['diaLocal']):
		if timesql == str(event['horaLocal']): 
			if float(resultados[0]['magnitud']) == eventmag:
				actualiza = 0		
				if verbose: print "\tSame event already in db " + str(resultados[0]['magnitud']) + '/' + str(eventmag)
		
        if actualiza == 1 and str(resultados[0]['fechalocal']) == str(event['diaLocal']) and mysqlhrastr == jsonhra[0] and mysqlhra[1] == jsonhra[1]:
		 # Codigo para preveer que no se trata del mismo sismo(revisado) con variacion de segundos
   		if int(jsonhra[2])> int(mysqlhra[2]):
			restmj= int(jsonhra[2
]) - int(mysqlhra[2])
   		else:
			restmj= int(mysqlhra[2]) -int(jsonhra[2])
		if restmj < 10 or resultados[0]['magnitud'] != eventmag:
